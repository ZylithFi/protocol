...
const LIQUIDITY_SLICE_COMMITMENT_DOMAIN_HEX: &str =
    "0x02bc4890bf4accafb4b8f647c96e53cdd702a9b2e8b405a4d4b83b87ef46c69a";
...
pub fn build_stwo_serialized_input(
    ...
    let liquidity_slice_commitments = witness
        .matched_order_witnesses
        .iter()
        .map(|_| "0x0".to_string())
        .collect::<Vec<_>>();
    let liquidity_slice_point_counts = witness
        .matched_order_witnesses
        .iter()
        .map(|_| "0x0".to_string())
        .collect::<Vec<_>>();
    let liquidity_slice_prices = Vec::new();
    let liquidity_slice_base_amounts = Vec::new();
    ...
    let mut payload = vec![
        ...
        LIQUIDITY_SLICE_COMMITMENT_DOMAIN_HEX.into(),
    ...
    push_span(&mut payload, &liquidity_slice_commitments);
    push_span(&mut payload, &liquidity_slice_point_counts);
    push_span(&mut payload, &liquidity_slice_prices);
    push_span(&mut payload, &liquidity_slice_base_amounts);
    push_span(
        &mut payload,
        &witness
            .matched_order_witnesses
...
fn auction_proof_vectors(
    ...
    let liquidity_slice_commitments = all_orders
        .iter()
        .map(|_| "0x0".to_string())
        .collect::<Vec<_>>();
    let liquidity_slice_point_counts = all_orders
        .iter()
        .map(|_| "0x0".to_string())
        .collect::<Vec<_>>();
    let liquidity_slice_prices = Vec::new();
...
struct AuctionProofVectors {
    ...
    liquidity_slice_commitments: Vec<String>,
    liquidity_slice_point_counts: Vec<String>,
    liquidity_slice_prices: Vec<String>,
    liquidity_slice_base_amounts: Vec<String>,
...
pub fn build_auction_result_serialized_input(
    ...
    push_span(&mut payload, &vectors.liquidity_slice_commitments);
    push_span(&mut payload, &vectors.liquidity_slice_point_counts);
    push_span(&mut payload, &vectors.liquidity_slice_prices);
    push_span(&mut payload, &vectors.liquidity_slice_base_amounts);
...
fn push_admission_order_vectors(payload: &mut Vec<String>, vectors: &AuctionProofVectors) {
    ...
    push_span(payload, &vectors.liquidity_slice_commitments);
    push_span(payload, &vectors.liquidity_slice_point_counts);
    push_span(payload, &vectors.liquidity_slice_prices);
    push_span(payload, &vectors.liquidity_slice_base_amounts);
...
fn admission_root_from_vectors(vectors: &AuctionProofVectors) -> Result<String, ProtocolError> {
    ...
    let same_len_vectors = [
        ...
        vectors.liquidity_slice_commitments.len(),
    ...
    for index in 0..order_count {
        let leaf_inputs = [
            ...
            vectors.liquidity_slice_commitments[index].as_str(),
...
mod tests {
    ...
    use crate::types::{
        ...
        LiquiditySlice, LiquiditySlicePoint, output_bundle_bucket_size,
    ...
    fn liquidity_attribution_artifact_roundtrip_recovers_band_details() {
        ...
        let plaintext = LiquidityAttributionPlaintext {
            ...
            curve_commitment: curve.commitment().expect("liquidity slice commitment"),
    ...
    fn stwo_serialized_input_binds_multiple_funding_inputs() {
        ...
        let _matched_liquidity_slice_commitments = read_serialized_span(&serialized, &mut index);
        let _matched_liquidity_slice_point_counts = read_serialized_span(&serialized, &mut index);
        let _matched_liquidity_slice_prices = read_serialized_span(&serialized, &mut index);
        let _matched_liquidity_slice_base_amounts = read_serialized_span(&serialized, &mut index);
    ...
    fn stwo_serialized_input_uses_consistent_asset_and_owner_encodings() {
        ...
        let matched_liquidity_slice_commitments = read_serialized_span(&serialized, &mut index);
        let matched_liquidity_slice_point_counts = read_serialized_span(&serialized, &mut index);
        let matched_liquidity_slice_prices = read_serialized_span(&serialized, &mut index);
        let matched_liquidity_slice_base_amounts = read_serialized_span(&serialized, &mut index);
        let _matched_limit_prices = read_serialized_span(&serialized, &mut index);
        let _matched_order_amounts = read_serialized_span(&serialized, &mut index);
        let _matched_min_fills = read_serialized_span(&serialized, &mut index);
        ...
        assert_eq!(matched_liquidity_slice_commitments, vec!["0x0".to_string()]);
        assert_eq!(
            matched_liquidity_slice_point_counts,
            vec!["0x0".to_string()]
        );
        assert!(matched_liquidity_slice_prices.is_empty());
        assert!(matched_liquidity_slice_base_amounts.is_empty());
    ...
    fn admission_serialized_input_binds_full_order_preimages() {
        ...
        let liquidity_slice_commitments = read_serialized_span(&serialized, &mut index);
        let liquidity_slice_point_counts = read_serialized_span(&serialized, &mut index);
        let liquidity_slice_prices = read_serialized_span(&serialized, &mut index);
        let liquidity_slice_base_amounts = read_serialized_span(&serialized, &mut index);
        ...
        assert_eq!(liquidity_slice_commitments, vec!["0x0".to_string()]);
        assert_eq!(liquidity_slice_point_counts, vec!["0x0".to_string()]);
        assert!(liquidity_slice_prices.is_empty());
        assert!(liquidity_slice_base_amounts.is_empty());
