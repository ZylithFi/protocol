...
use crate::{
    AssetId, BatchId, LiquidityAttributionPlaintext, LiquidityBandAttribution,
    LiquidityPositionCommitment, LiquidityPositionCurveKind, LiquidityPositionFillDelta,
    LiquidityPositionRootTransition, LiquidityPositionStatus, LiquidityPositionTransitionKind,
    LiquiditySlice, LiquiditySlicePoint, Note, NoteCommitment, OrderSide, PrivateLiquidityPosition,
...
pub struct LiquidityPositionCurveSlice {
    ...
    pub bid: Option<LiquiditySlice>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ask: Option<LiquiditySlice>,
    #[serde(with = "crate::types::serde_u128_decimal")]
...
fn build_bid_curve(
    ...
    price_base_scale: u128,
    seed: u128,
) -> Result<Option<LiquiditySlice>, ProtocolError> {
    ...
    for (price, quote_amount) in prices.into_iter().zip(allocations) {
        ...
        if base_amount > 0 {
            points.push(LiquiditySlicePoint { price, base_amount });
        }
    }
...
fn build_ask_curve(
    ...
    let mut points = prices
        ...
        .filter_map(|(price, base_amount)| {
            (base_amount > 0).then_some(LiquiditySlicePoint { price, base_amount })
        })
        .collect::<Vec<_>>();
    cap_curve_points(&mut points, position.max_fill_base_per_batch)?;
    curve_or_none(points)
...
fn cap_curve_points(
    points: &mut Vec<LiquiditySlicePoint>,
    max_total_base: u128,
) -> Result<(), ProtocolError> {
...
fn curve_or_none(
    points: Vec<LiquiditySlicePoint>,
) -> Result<Option<LiquiditySlice>, ProtocolError> {
    if points.len() < crate::types::MIN_LIQUIDITY_SLICE_POINTS {
        return Ok(None);
    }
    let curve = LiquiditySlice { points };
    curve.validate()?;
    Ok(Some(curve))
}
