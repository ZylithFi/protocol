...
pub const MIN_LIQUIDITY_SLICE_POINTS: usize = 3;
pub const MAX_LIQUIDITY_SLICE_POINTS: usize = 8;
...
pub const LIQUIDITY_SLICE_MIN_SPREAD_BPS_STABLE_CONVERSION: u128 = 5;
pub const LIQUIDITY_SLICE_MIN_SPREAD_BPS_CONVERSION: u128 = 10;
pub const LIQUIDITY_SLICE_MIN_SPREAD_BPS_SPECULATIVE: u128 = 20;
...
pub struct LiquiditySlicePoint {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LiquiditySlice {
    pub points: Vec<LiquiditySlicePoint>,
}
...
impl LiquiditySlice {
    pub fn commitment(&self) -> Result<String, ProtocolError> {
        ...
        Ok(poseidon_chain_hex(
            felt_from_hex_str(LIQUIDITY_SLICE_COMMITMENT_DOMAIN_HEX)?,
            &fields,
        ))
    }
    ...
    pub fn total_base_amount(&self) -> Result<u128, ProtocolError> {
        self.points.iter().try_fold(0u128, |total, point| {
            total.checked_add(point.base_amount).ok_or_else(|| {
                ProtocolError::InvalidOrder("liquidity slice base amount overflows u128".into())
            })
        })
    }

    pub fn validate(&self) -> Result<(), ProtocolError> {
        if self.points.len() < MIN_LIQUIDITY_SLICE_POINTS {
            return Err(ProtocolError::InvalidOrder(format!(
                "liquidity slice must contain at least {MIN_LIQUIDITY_SLICE_POINTS} points"
            )));
        }
        if self.points.len() > MAX_LIQUIDITY_SLICE_POINTS {
            return Err(ProtocolError::InvalidOrder(format!(
                "liquidity slice uses {} points, maximum is {}",
                self.points.len(),
        ...
        for point in &self.points {
            if point.price == 0 || point.base_amount == 0 {
                return Err(ProtocolError::InvalidOrder(
                    "liquidity slice prices and base amounts must be positive".into(),
                ));
            }
            if previous_price.is_some_and(|price| point.price <= price) {
                return Err(ProtocolError::InvalidOrder(
                    "liquidity slice points must be strictly increasing by price".into(),
                ));
            }
...
impl LiquidityPositionCurvePolicy {
    pub fn validate(&self) -> Result<(), ProtocolError> {
        if self.band_count < MIN_LIQUIDITY_SLICE_POINTS as u64
            || self.band_count > MAX_LIQUIDITY_SLICE_POINTS as u64
        {
            return Err(ProtocolError::InvalidOrder(format!(
                "liquidity position band count must be between {MIN_LIQUIDITY_SLICE_POINTS} and {MAX_LIQUIDITY_SLICE_POINTS}"
            )));
        }
...
impl OrderIntent {
    pub fn commitment(&self) -> Result<OrderCommitment, ProtocolError> {
        ...
        let liquidity_slice_commitment = field_from_u64(0);
...
pub fn liquidity_slice_min_spread_bps(pair_id: &PairId) -> u128 {
    match pair_id.0.as_str() {
        "USDC/USDT" => LIQUIDITY_SLICE_MIN_SPREAD_BPS_STABLE_CONVERSION,
        _ if is_conversion_pair(pair_id) => LIQUIDITY_SLICE_MIN_SPREAD_BPS_CONVERSION,
        _ => LIQUIDITY_SLICE_MIN_SPREAD_BPS_SPECULATIVE,
    }
}

pub fn liquidity_slice_min_band_base_amount(pair_id: &PairId) -> u128 {
    match pair_id.0.as_str() {
...
pub fn default_min_order_amount(pair_id: &PairId) -> u128 {
    liquidity_slice_min_band_base_amount(pair_id)
}
...
mod tests {
    use super::{
        ...
        LiquidityAttributionReceipt, LiquidityPositionBacking, LiquidityPositionCurveKind,
        LiquidityPositionCurvePolicy, LiquidityPositionFillDelta, LiquidityPositionOracleGuard,
        LiquidityPositionRotationPolicy, LiquidityPositionStatus, LiquiditySlice,
        LiquiditySlicePoint, MAX_LIQUIDITY_SLICE_POINTS, MatchedOrderWitness,
        NOTE_RECOGNITION_ALGORITHM, Note, NoteCommitment, NoteConsolidationWitness,
        NoteMembershipKind, NoteMembershipWitness, Nullifier, NullifierHistoryBatch,
        OUTPUT_NOTE_CIPHERTEXT_LEN, OrderCancellationRequest, OrderCommitment,
        OrderExecutionReport, OrderIntent, OrderShareBundle, OrderSide, OrderSubmission, OrderType,
        OutputCiphertextBundle, OutputNoteMerkleProof, OutputNoteRecord, OutputRecoveryRecord,
        PairId, PreparedBatchStatus, PrivateExecutionKeyPrivateConfig, PrivateLiquidityPosition,
        PrivateOrderPayload, PrivateSettlementReportQuery, ProductConfig,
        ...
        StarknetPrivacyFundingRail, TRANSCRIPT_SHAPE_POLICY_VERSION, TimeInForce,
        TrustedOrderIngressRequest, WalletSnapshot, funding_input_set_commitment,
    ...
    #[test]
    fn liquidity_slice_rejects_too_many_points() {
        let slice = LiquiditySlice {
            points: (0..=MAX_LIQUIDITY_SLICE_POINTS)
                .map(|index| LiquiditySlicePoint {
